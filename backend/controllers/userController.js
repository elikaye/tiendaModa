...
const registrarUsuario = async (req, res) => {
  try {
    ...
    if (existeUsuario) return res.status(400).json({ success:false, message:'Email ya registrado' });
...
const loginUsuario = async (req, res) => {
  try {
    ...
    if (!passwordOk) return res.status(401).json({ success:false, message:'Credenciales inválidas' });
    ...
    return res.json({ success:true, message:'Login exitoso', user:usuarioSinPassword, token });
